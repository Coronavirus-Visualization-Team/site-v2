...
const TeamPage = (props) => {
  ...
  return (
    ...
    <Text sx={ { variant: "styles.headerText", mb: 4, mt: 4, alignSelf: 'center', paddingTop: '10px !important', fontWeight: '700', textAlign: 'center', cursor: 'pointer', display: 'flex' } } onClick={() => setCommManagers(!commManagers)}>Community Managers <span sx={{ fontSize: '0.4em', my: 'auto', ml: 3 }}>{commManagers ? '▲' : '▼'}</span></Text>
      <Text
        sx={ { variant: "styles.bodyText", color: "secondary" } }
      >

      {/** width={[ 128, null, 192 ]} */}
      <Grid columns={[ 2, null, 4 ]} sx={{ height: `${commManagers ? 'unset' : '0px'}`, transform: `scaleY(${commManagers ? '1' : '0'})` }}>
        {teamData.map((item) => {
            if (item.node.childMarkdownRemark) {
              const data = item.node.childMarkdownRemark.frontmatter;
